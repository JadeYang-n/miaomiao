...
static int sprite_x = 45;
static int sprite_y = 45;

/* 测试用：标记是否用简单矩形测试 */
static bool use_rect_test = false;  /* 关闭矩形测试，恢复猫精灵 */

/* 单次绘制测试 - 恢复分批 */
static bool single_draw_test = false;
...
/* 音频活动标志 - 音频传输期间暂停动画更新 */
static volatile bool g_audio_active = false;
...
/* 测试用：画纯色诊断 - 现在由 init 接管 */
static void draw_color_test(void)
{
    if (!display_initialized) return;
    /* 清屏已在 init 中完成，这里只打印日志 */
    ESP_LOGI(TAG, "白屏已绘制（init中完成）");
}
...
esp_err_t display_init(void)
{
    ...
    esp_lcd_panel_io_spi_config_t io_cfg = {
        ...
        .spi_mode = 3,                    /* 改回模式3，匹配nopnop2002参考代码 */
        .pclk_hz = 40 * 1000 * 1000,  /* 40MHz，与 xiaozhi-esp32 配置一致 */
        .trans_queue_depth = 10,
        .lcd_cmd_bits = 8,
        .lcd_param_bits = 8,
        .flags = {{0}},                   /* 先尝试不加SPI_DEVICE_NO_DUMMY */
    };
    ESP_LOGI(TAG, "开始 esp_lcd_new_panel_io_spi...");
    ...
    ESP_LOGI(TAG, "esp_lcd_new_panel_io_spi 成功");

    /* 初始化 ST7789 面板 */
    esp_lcd_panel_dev_config_t panel_cfg = {
        .reset_gpio_num = OLED_RST_PIN,
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,  /* RGB 颜色顺序 */
        .data_endian = LCD_RGB_DATA_ENDIAN_LITTLE,   /* 保持 LITTLE */
        .bits_per_pixel = 16,
    };
    ESP_LOGI(TAG, "开始 esp_lcd_new_panel_st7789...");
    ...
    ESP_LOGI(TAG, "ST7789 初始化完成，等待200ms...");
    vTaskDelay(pdMS_TO_TICKS(200));

    ESP_LOGI(TAG, "测试白色矩形...");
    /* 测试：用白色矩形填充整个屏幕 */
    uint16_t white_buf[ST7789_WIDTH];
    for (int i = 0; i < ST7789_WIDTH; i++) {
        white_buf[i] = 0xFFFF;  // 纯白 RGB565
    }
    for (int y = 0; y < ST7789_HEIGHT; y++) {
        esp_lcd_panel_draw_bitmap(panel_handle, 0, y, ST7789_WIDTH, y + 1, white_buf);
    }
    ESP_LOGI(TAG, "白色矩形完成");
    vTaskDelay(pdMS_TO_TICKS(100));

    printf("ST7789 init SUCCESS, returning\n");
    printf("========================================\n");
...
static void animation_task(void *arg)
{
    ...
    while (1) {
        ...
        /* 检查是否到了眨眼时间（只有在非空闲、非常告警、非音频活动状态时） */
        if (!is_blinking && now >= next_blink_time_ms && !showing_idle && !g_audio_active) {
            /* 开始眨眼 */
            is_blinking = true;
...
static void display_init_animation(void)
{
    ...
    BaseType_t ret = xTaskCreatePinnedToCore(
        ...
        4096,
        NULL,
        3,  /* 优先级：低于音频输入(8)，高于空闲任务 */
        &animation_task_handle,
        1  /* Core 1：避免与音频任务争抢 Core 0 */
    );
...
void display_show_status(const char *line1, const char *line2)
{
    ...
    display_set_wifi(wifi_st.connected);

    last_draw_time = xTaskGetTickCount() * portTICK_PERIOD_MS;
    content_drawn = true;
    cat_face_drawn = false;
}
...
void display_show_message(const char *speaker, const char *message)
{
    ...
    fill_rect(0, 0, ST7789_WIDTH, ST7789_HEIGHT, COLOR_BLACK);  // 清除
    fill_rect(2, 2, ST7789_WIDTH - 4, ST7789_HEIGHT - 4, COLOR_WHITE);  // 白色背景
...
void display_set_wifi(bool connected)
{
    ...
    /* WiFi 图标绘制在右上角 (224, 4)，16x16 */
    int wx = 224;
    int wy = 4;

    /* 先清除图标区域 16x16 */
    fill_rect(wx, wy, 16, 16, COLOR_WHITE);

    if (connected) {
        /* WiFi 已连接 - 绿色图标 */
        /* 第4行: (4,4)-(11,4) */
        fill_rect(wx+4, wy+4, 8, 1, 0x2CE9);
        fill_rect(wx+3, wy+5, 2, 1, 0x252A);
        fill_rect(wx+11, wy+5, 2, 1, 0x24EA);
        fill_rect(wx+2, wy+6, 1, 1, 0x250A);
        fill_rect(wx+6, wy+6, 1, 1, 0x2509);
        fill_rect(wx+7, wy+6, 3, 1, 0x252A);
        fill_rect(wx+13, wy+6, 1, 1, 0x252A);
        fill_rect(wx+4, wy+7, 2, 1, 0x2509);
        fill_rect(wx+10, wy+7, 2, 1, 0x2509);
        fill_rect(wx+4, wy+8, 1, 1, 0x252A);
        fill_rect(wx+7, wy+8, 2, 1, 0x2509);
        fill_rect(wx+11, wy+8, 1, 1, 0x24EA);
        fill_rect(wx+5, wy+9, 1, 1, 0x14E8);
        fill_rect(wx+6, wy+9, 1, 1, 0x252A);
        fill_rect(wx+9, wy+9, 1, 1, 0x250A);
        fill_rect(wx+10, wy+9, 1, 1, 0x1CE8);
        fill_rect(wx+7, wy+10, 2, 1, 0x2509);
        fill_rect(wx+7, wy+11, 2, 1, 0x250A);
    } else {
        /* WiFi 未连接 - 红色图标 */
        fill_rect(wx+4, wy+5, 2, 1, 0xE0E4);
        fill_rect(wx+6, wy+5, 1, 1, 0xE903);
        fill_rect(wx+9, wy+5, 2, 1, 0xE904);
        fill_rect(wx+3, wy+6, 1, 1, 0xE124);
        fill_rect(wx+6, wy+6, 3, 1, 0xD903);
        fill_rect(wx+9, wy+6, 1, 1, 0xE8E3);
        fill_rect(wx+11, wy+6, 2, 1, 0xE925);
        fill_rect(wx+5, wy+7, 1, 1, 0xE124);
        fill_rect(wx+10, wy+7, 2, 1, 0xE924);
        fill_rect(wx+6, wy+8, 1, 1, 0xE125);
        fill_rect(wx+7, wy+8, 3, 1, 0xD924);
        fill_rect(wx+7, wy+10, 2, 1, 0xD924);
        fill_rect(wx+7, wy+11, 1, 1, 0xC925);
        fill_rect(wx+8, wy+11, 1, 1, 0xEBF0);
    }
}
...
/**
 * 设置音频活动状态（音频活动期间跳过眨眼，但允许表情切换）
 */
void display_set_audio_active(bool active)
{
    g_audio_active = active;
    /* 进入音频活动时，如果正在眨眼，强制恢复正常 */
    if (active && is_blinking) {
        is_blinking = false;
        current_expression = target_expression;
    }
    /* 退出音频活动时，重置眨眼计时器 */
